...
@objc(KeolisAccountManagement) public class KeolisAccountManagement : NSObject, AccountManagement {
    ...
    internal var _oldLogin : String { // old login stocked for migration from adm v2
        get {
            return UserDefaults.standard.string(forKey: "__NavitiaSDKPartners_KeolisAccountManagement_oldLogin") ?? ""
        }
        
        set {
            UserDefaults.standard.set(newValue, forKey: "__NavitiaSDKPartners_KeolisAccountManagement_oldLogin")
        }
    }
    
    internal var _oldEncryptedPassword : String { // old encrypted password for migration from adm v2
        get {
            return UserDefaults.standard.string(forKey: "__NavitiaSDKPartners_KeolisAccountManagement_oldEncryptedPassword") ?? ""
        }
        
        set {
            UserDefaults.standard.set(newValue, forKey: "__NavitiaSDKPartners_KeolisAccountManagement_oldEncryptedPassword")
        }
    }
    
    public func setOldAccount(oldLogin: String, oldEncryptedPassword: String) {
        _oldLogin = oldLogin
        _oldEncryptedPassword = oldEncryptedPassword
    }
    
    public func forceClearOldAccount() {
        _oldLogin = ""
        _oldEncryptedPassword = ""
    }
    
    internal func automaticMigrate( callbackSuccess : @escaping () -> Void, callbackError : @escaping (Int, [String: Any]?) -> Void) {
        
        if _oldEncryptedPassword.isEmpty && _oldLogin.isEmpty {
            callbackSuccess()
            return
        }
        
        migrate(username: _oldLogin, password: _oldEncryptedPassword, callbackSuccess: ({ () in
            self._oldLogin = ""
            self._oldEncryptedPassword = ""
            callbackSuccess()
    ...
    public func createAccount( callbackSuccess: @escaping () -> Void,
        ...
        if !(_oldLogin.isEmpty) && !(_oldEncryptedPassword.isEmpty) {
            automaticMigrate(callbackSuccess: {
                callbackSuccess()
                return
            }) { (statusCode, data) in
                if NavitiaSDKPartnersReturnCode(rawValue: statusCode) != nil {
            ...
            NavitiaSDKPartnersRequestBuilder.soapPost(stringUrl: "\((accountConfiguration as! KeolisAccountManagementConfiguration).url)/socle/\((accountConfiguration as! KeolisAccountManagementConfiguration).network)/CreateAccount/V1.0/", header: _getKeolisHeaderWS(), soapMessage: createAccountAnonymousXML(password: password.encrypt(key: (_keolisConfiguration?.hmacKey)!))) { success, statusCode, data in
                ...
                if success {
                    ...
                    print( email )
                    print( password.encrypt(key: (self._keolisConfiguration?.hmacKey)!) )
    ...
    public func migrate( username: String, password: String,
        ...
        if password.isEmpty || username.isEmpty || !(NavitiaSDKPartnersExtension.isValidEmail(str: username)) {
            ...
            data["details"] = details
            callbackError(NavitiaSDKPartnersReturnCode.failedToMigrate.getCode(), NavitiaSDKPartnersReturnCode.failedToMigrate.getError())
            return
        }
