...
open class FloatRatingView: UIView {
    ...
    /// Set the space between stars
    @IBInspectable open var starsInterspace: CGFloat = 0 {
        didSet {
            if starsInterspace != oldValue {
                refresh()
            }
        }
    }
    ...
    override open func layoutSubviews() {
        ...
        let containerFrameWidth: CGFloat = frame.size.width - (4 * starsInterspace)
        let desiredImageWidth = containerFrameWidth / CGFloat(emptyImageViews.count)
        ...
        let imageXOffset = (containerFrameWidth - (imageViewSize.width * CGFloat(emptyImageViews.count))) / CGFloat((emptyImageViews.count - 1))
        
        for i in 0..<maxRating {
            let imageFrame = CGRect(x: i == 0 ? 0 : CGFloat(i)*(imageXOffset + imageViewSize.width + starsInterspace), y: 0, width: imageViewSize.width, height: imageViewSize.height)
