...
func durationText(bundle: Bundle, seconds: Int32, useFullFormat: Bool = false) -> String {
    if (seconds < 60) {
        return String(format: "%@ %@", NSLocalizedString("less_than_a", bundle: bundle, comment: ""), NSLocalizedString("units_minute", bundle: bundle, comment: "Units minute"))
    } else if (seconds < 120) {
        return String(format: "1 %@", NSLocalizedString("units_minute", bundle: bundle, comment: "Units minute"))
    } else if (seconds < 3600) {
        let minutes: Int32 = seconds / 60
        return String(format: "%d %@", minutes, NSLocalizedString("units_minutes", bundle: bundle, comment: "Units minutes"))
    } else {
        let hours: Int32 = seconds / 3600
        let remainingMinutes: Int32 = (seconds / 60) - (hours * 60)
        var minutes: String = String(format: "%d", remainingMinutes)
        if remainingMinutes < 10 {
            minutes = String(format: "0%d", remainingMinutes)
        }
        if useFullFormat {
            if hours > 1 {
            ...
            return String(format: "%d%@%@", hours, NSLocalizedString("units_h", bundle: bundle, comment: "Units hour abbr"), minutes)
        }
    }
}
...
func distanceText(bundle: Bundle, meters: Int32) -> String {
    if (meters < 1000) {
        return String(format: "%d %@", meters, NSLocalizedString("units_meters", bundle: bundle, comment: "Units meters"))
    } else {
        let formatter: NumberFormatter = NumberFormatter()
        formatter.maximumFractionDigits = 1
        let distance: String = formatter.string(for: (Float(meters) / 1000))!
        return String(format: "%@ %@", distance, NSLocalizedString("units_km", bundle: bundle, comment: "Units km"))
    }
}
